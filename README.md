# Volný projekt

> **Jméno a příjmení:** `Mikoláš Valoušek`

> **Název projektu:** `Mikešovy lahůdky`

---

## O projektu

_Krátce popište svůj projekt – čemu se věnuje, co nabízí a jaký problém řeší?_

`  Projekt se zabíva "prodejem" mých produktů a zkušeností v oblasti cukrařiny, např...Prodej dortů, recepty, možná nejaký přeprodej pomůcek k práci.  `

---

## Konkurence / Inspirace

_Uveďte konkurenční projekty. Co se vám na nich líbí, co ne? V čem se můžete inspirovat?_

` https://www.frutiko.cz/cukrarna-liberec ... líbí se mi přehlednost webu a to, že je jasně poznat k čemu web slouží,..Moc se mi nelíbí zelená barva, sice to vypadá hezky, ale nejsem si jistý jestli to je uplně tématické k danému tématu ... Můžu se inspirovat v "headeru" a "footeru" kde to bude nejspíše dost podobné `

---

## Cílová skupina

_Kdo je váš cílový uživatel / zákazník? Popište typického zástupce cílové skupiny._

` Lidé co se neradi dělají se zákusky na např... oslavy... nebo Obézní lidé `


## Odkazy a výstupy

_Shrnutí všech odkazů na jednom místě. Průběžně aktualizujte._

| Výstup | Odkaz |
|---|---|
| GitHub Pages (live) | [`https://github.com/pslib-cz/2025-p2a-web-volny-projekt-MikolasValousek09/actions/runs/24797499116`](#) |
| Grafický manuál (Figma / Canva) | [`Doplň odkaz`](#) |
| Grafický návrh webu (Figma) | [`https://www.figma.com/design/VjdgeVXpEupt9CDPTIzIHb/Valou%C5%A1ekMikol%C3%A1%C5%A1?node-id=0-1&p=f&t=YAr9nLfoxHEWOW7i-0`](#) |
| Fotografie / média | [`Doplň odkaz`](#) |

---

## Poznámky

_Prostor pro vlastní poznámky, zpětnou vazbu, TODO apod._

`.`